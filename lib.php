...
{
    global $DB;
    $sql = "SELECT distinct(user.id), user.firstname, user.lastname, user.username";
    $sql .= " FROM {user} user JOIN {block_bcgt_user_qual} userqual ON userqual.userid = user.id 
    ...
    if($options || count($roles) > 0)
    {
        ...
        if($myStudents)
        {
            ...
            {
                $sql .= " JOIN {context} c ON c.instanceid = user.id 
...
function get_users_bcgt($search = '', $userID = -1)
{
    ...
    if($search != '')
    {
        $sql .= " (user.firstname LIKE ? OR user.lastname LIKE ? 
            OR user.email LIKE ? OR user.username LIKE ?)";
        $params[] = '%'.$search.'%';
        $params[] = '%'.$search.'%';
        $params[] = '%'.$search.'%';
...
WHERE course.id = ? AND r.shortname = ?  AND (ra.component = '' OR ra.component = 'enrol_database')
UNION
...
{
    global $DB;
    $sql = "SELECT distinct user.* FROM {user} user 
        JOIN {block_bcgt_user_qual} userqual ON userqual.userid = user.id 
...
//    }
    $sql .= " WHERE user.deleted != ? AND userqual.roleid = ?";
...
function bcgt_student_search_db($search, &$params)
{
    ...
    $sql = '';
    $sql .= " user.username LIKE ? OR user.firstname LIKE ? OR user.lastname LIKE ? OR user.email LIKE ?";
    $params[] = '%'.$search.'%';
    $params[] = '%'.$search.'%';
    $params[] = '%'.$search.'%';
    $params[] = '%'.$search.'%';
    ...
    {
        $searchSplit = explode(' ', $search);
        $sql .= " OR user.firstname LIKE ? AND user.lastname LIKE ?";
...
function bcgt_display_qual_assessments($qualID, $search = '', $userID = -1, $assID = -1, $groupingID = -1)
{
    ...
    if($search != '' || $userID != -1 || $assID != -1 || $groupingID != -1 || $qualID != -1)
    {
        ...
        if($userID != -1)
        {
            ...
            $and = true;
            $sql .= ' role.shortname = ? || role.shortname = ? AND roleass.userid = ?';
            $params[] = 'editingteacher';
            $params[] = 'teacher';
            $params[] = $userID;;
        }
...
function bcgt_get_users_on_coursemodules($qualID = -1,$courseID = -1, $groupingID = -1, $actID = -1)
{
    ...
    $sqlUnion = " UNION ";
    $sql2 = " SELECT distinct(user.id), user.* FROM {user} user 
        JOIN {block_bcgt_user_qual} userqual ON userqual.userid = user.id 
        JOIN {role_assignments} ra ON ra.userid = user.id 
        JOIN {context} con ON con.id = ra.contextid 
        JOIN {course} course ON course.id = con.instanceid 
...
function bcgt_apply_assignment_grading_box(&$mform, $instance, $mod, $userID)
{
    ...
    $mform->addElement('header', 'gradetrackerheader', get_string('gradetracker2', 'block_bcgt'));
        ...
        foreach($activityRefs as $unitID => $criteria)
        {
            if ($criteria)
            {
                ...
                if ($unit)
                {
                    ...
                    $mform->addElement('html', '<b>' . $unit->get_display_name() . '</b><br>');
