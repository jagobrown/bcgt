...
if ($unit){
    $PAGE->navbar->add($unit->get_name(),$CFG->wwwroot.'/blocks/bcgt/grids/unit_grid.php?uID='.$unitID.'&qID='.$qualID,'title');
}
...
if ($unit && method_exists($unit, 'import_unit_grid')){
    ...
    echo "<p class='c'>";
        echo "<input type='button' class='btn' value='Back to Grid' onclick='window.location.href=\"{$CFG->wwwroot}/blocks/bcgt/grids/unit_grid.php?uID={$unitID}&qID={$qualID}\"' />";
    echo "</p>";
